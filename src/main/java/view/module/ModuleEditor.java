...
public class ModuleEditor {
	...
	public static boolean display(ModuleDao moduleDao, Module module) {
		JPanel panel = new JPanel(new GridLayout(0, 1));

		lbName = new JLabel("Tên học phần");
		panel.add(lbName);
		tfName = new JTextField(module.getName());
		panel.add(tfName);

		lbCredit = new JLabel("Số tín chỉ");
		tfCredit = new JTextField(module.getCredit() + "");
		panel.add(lbCredit);
		panel.add(tfCredit);

		lbQuantity = new JLabel("Số lượng");
		tfQuantity = new JTextField(module.getQuantity() + "");
		panel.add(lbQuantity);
		panel.add(tfQuantity);

		lbRegistered = new JLabel("Đã đăng ký");
		tfRegistered = new JTextField(module.getRegistered() + "");
		panel.add(lbRegistered);
		panel.add(tfRegistered);

		lbDescription = new JLabel("Mô tả");
		tfDescription = new JTextField(module.getDescription());
		panel.add(lbDescription);
		panel.add(tfDescription);

		int result = JOptionPane.showConfirmDialog(null, panel, "Sửa học phần", JOptionPane.OK_CANCEL_OPTION,
		...
		if (result == JOptionPane.OK_OPTION) {
			try {
				...
				return moduleDao.update(new Module(module.getId(), name, credit, quantity, registered, description));
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
